 ...
 */
export const useActivityState = (options?: { canRestart?: boolean, initialState?: ActivityState }) => {
    const [state, setState] = useState<ActivityState>(options?.initialState ?? 'notStarted');
     ...
     * @returns new state.
     */
    const toggleState = (callback?: ToggleActivityCallback) => setState((currentState) => {
        switch(currentState) {
            case 'notStarted':
